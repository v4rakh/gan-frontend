		...
		if (address) {
			...
			subscriptionService
				...
				.then(() => {
					...
					dispatch(alertActions.success(t('page.subscription_create.form.success', { address: address })));
				})
				.catch((error) => {
					setLoading(false);
	...
	return (
		<Container>
			<Row className="justify-content-md-center">
				<Col md={6}>
					<Card>
						<Card.Header>{t('page.subscription_create.form.title')}</Card.Header>
						<Card.Body className="card-body">
							<Form name="form" onSubmit={handleSubmit}>
								<Form.Group>
									<Form.Label>{t('page.subscription_create.form.address')}</Form.Label>
									<Form.Control
										type="text"
										name="address"
										value={address}
									...
									/>
									{submitted && !address && (
										<div className="invalid-feedback">{t('page.subscription_create.form.address_hint')}</div>
									)}
								</Form.Group>
								<Form.Group>
									<div className="row">
										<div className="col-6 col-sm-6">
											<Button type="submit" className="float-left">
												...
												{t('page.subscription_create.form.button_title')}
											</Button>
										</div>
										<SubscriptionLinks showCreate={false} />
									</div>
								</Form.Group>
							</Form>
						</Card.Body>
